...
cat $1 | sed -e 1,3d | sed -e 's/^[ \t]*//' | tr -s " " | sed 's| |\t|g' | sed 's/(//g' | sed 's/)//g' > file_work_with1.txt

# 我们在对 file_work_with.txt 进行处理，将RepeatMasker未能连接的片段进行连接
SCRIPT_DIR=$(dirname "$0")
# 调用同一目录下的 Python 脚本
python "$SCRIPT_DIR/connect_BM_RM2.py" --in_file file_work_with1.txt --std_lib $2 --test_lib $3
