...
3. **longest_repeats_*.fa** represents the output of the FMEA algorithm, while **longest_repeats_*.flanked.fa** extends the sequences at both ends of **longest_repeats_*.fa**.
4. **confident_tir_*.fa**, **confident_helitron_*.fa**, **confident_non_ltr_*.fa** represent the identification results of the TIR, Helitron, and non-LTR modules in HiTE respectively, while **confident_other_*.fa** indicates the identification results of the homology-based non-LTR searching module.
5. Note that "*" represents the number of blocks that the genome is divided into.
...
6. The **HiTE.out**, **HiTE.gff**, and **HiTE.tbl** files are generated using parameter `--annotate 1`. 
...
5. The updated **RepeatMasker.lib** file should be around **133 MB** in size.
