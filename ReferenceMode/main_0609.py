...
# backjob.start()
    ...
    # repeats_consensus = tmp_output_dir + '/repeats.cons.fa'
    # cd_hit_command = tools_dir + '/cd-hit-est -aS ' + str(0.95) + ' -c ' + str(0.95) + ' -G 0 -g 1 -A 80 -i ' + repeats_path + ' -o ' + repeats_consensus + ' -T 0 -M 0'
    ...
    for ref_name in ref_contigs_list.keys():
        ...
        for i in range(len(ref_list)):
            if ref_list[i] > filter_base_freq:
                if start_pos == -1:
                    start_pos = i
                end_pos = i
            elif start_pos != -1:
    ...
    connected_frags = connect_frags(connected_regions, filter1_repeats_path, reference, threads, tools_dir, tmp_output_dir, skip_threshold, RepeatMasker_Home)
    ...
    cd_hit_command = tools_dir + '/cd-hit-est -aS ' + str(0.95) + ' -c ' + str(0.95) + ' -G 0 -g 1 -A 80 -i ' + repeats_connected_file + ' -o ' + repeats_connected_consensus + ' -T 0 -M 0'
    log.logger.debug(cd_hit_command)
    os.system(cd_hit_command)
    ...
    if not os.path.exists(trf_dir):
        os.makedirs(trf_dir)
    (repeat_dir, repeat_filename) = os.path.split(repeats_connected_consensus)
    (repeat_name, repeat_extension) = os.path.splitext(repeat_filename)
    trf_command = 'cd ' + trf_dir + ' && ' + TRF_Path + ' ' + repeats_connected_consensus + ' 2 7 7 80 10 50 500 -f -d -m'
    ...
    repeats_contigNames, repeats_contigs = read_fasta(repeats_connected_consensus)
