...
if __name__ == '__main__':
    ...
    tmp_output_dir = output_dir + '/CRD.2022-06-29.10-59-3'
...
# backjob.start()
    ...
    #     job = ex.submit(get_longest_repeats, file, blast_program_dir, tools_dir, extend_base_threshold, max_single_repeat_len)
    #     jobs.append(job)
    ...
    # # with open(longest_repeats_path, 'w') as f_save:
    # #     for query_name in longest_repeats.keys():
    # #         seqs_items = longest_repeats[query_name]
    # #         for longest_seq_item in seqs_items:
    # #             confidence = longest_seq_item[0]
    # #             longest_seq = longest_seq_item[1]
    # #             log.logger.debug(confidence, len(longest_seq))
    ...
    #
    # filter_TE_path = tmp_output_dir + '/longest_repeats.cons.filter_tandem.fa'
    # longestRepeatNames, longestRepeatContigs = read_fasta(filter_TE_path)
    ...
    # filter_TE_path = tmp_output_dir + '/longest_repeats.cons.filter_tandem.fa'
    # orig_names, orig_contigs = read_fasta(filter_TE_path)
    # query_records = {}
    # blastn2Results_path = tmp_output_dir + '/longest_repeat.ref.out'
    ...
    #             full_len_records = 0
    #         if abs(q_start - 1) <= 10 and abs(q_end - query_len) <= 10 and identity >= 95:
    #             full_len_records += 1
    #         else:
    #             if not query_records.__contains__(query_name):
    #                 query_records[query_name] = {}
    ...
    #             if cluster_longest_query_len != -1:
    #                 longest_queries.append((cluster_longest_query_start, cluster_longest_query_end, cluster_longest_query_len))
    #                 if query_name == 'N_9825-len_990':
    #                     print(cur_cluster)
    #                     print((cluster_longest_query_start, cluster_longest_query_end, cluster_longest_query_len))
    #
    #     # is_false_positive = True
    #     # full_len_query_num = 0
    #     # for longest_query in longest_queries:
    #     #     if float(longest_query[2])/query_len >= 0.9:
    #     #         full_len_query_num += 1
    #     #         # if query_name == 'N_16493-len_778':
    #     #         #     print(longest_query)
    ...
    #     # elif false_positive_queries.__contains__(query_name):
    ...
    #         if float(longest_query[2])/query_len >= 0.9:
    #             full_len_query_num += 1
    #     if query_len < 2000 and full_len_query_num < 10:
    #         false_positive_queries[query_name] = 1
    #
    #
    #     if query_name == 'N_9825-len_990':
    #         print(full_len_query_num, len(longest_queries), float(full_len_query_num)/len(longest_queries), false_positive_queries.__contains__(query_name))
    #
    # # count = 0
    ...
    #
    # seq_count = 0
    # filter_TE_path = tmp_output_dir + '/longest_repeats.cons.filter_tandem.filter_fp.fa'
    # with open(filter_TE_path, 'w') as f_save:
    ...
    #     os.makedirs(tmp_blast_dir)
    #
    # (repeat_dir, repeat_filename) = os.path.split(filter_TE_path)
    #
    # longest_repeats_Names, longest_repeats_Contigs = read_fasta(filter_TE_path)
    #
    ...
    #         cur_contigs[item[0]] = item[1]
    #     store_fasta(cur_contigs, split_repeat_file)
    #     os.system('cp ' + filter_TE_path + ' ' + single_tmp_dir)
    ...
    # filter_TE_path_consensus = tmp_output_dir + '/longest_repeats.cons.filter_tandem.filter_fp.cons.fa'
    # cd_hit_command = tools_dir + '/cd-hit-est -aS ' + str(0.8) + ' -c ' + str(0.8) + ' -G 0 -g 1 -A 80 -i ' + filter_TE_path + ' -o ' + filter_TE_path_consensus + ' -T 0 -M 0'
    # log.logger.debug(cd_hit_command)
    # os.system(cd_hit_command)
                ...
                for j in range(i + 1, len(subject_pos)):
                    ...
                    if ext_frag[1] > longest_query_end:
                        ...
                        if longest_subject_start < longest_subject_end and ext_frag[2] < ext_frag[3]:
                            ...
                            if ext_frag[3] > longest_subject_end:
                                # forward extend
                                if ext_frag[0] - longest_query_end < extend_base_threshold and ext_frag[2] > longest_subject_end:
                                    # update the longest path
                                    longest_query_start = longest_query_start
                                    longest_query_end = ext_frag[1]
                                    longest_subject_start = longest_subject_start if longest_subject_start < \
                            ...
                            if ext_frag[3] < longest_subject_end:
                                # reverse extend
                                if ext_frag[0] - longest_query_end < extend_base_threshold and longest_subject_end > ext_frag[2]:
                                    # update the longest path
                                    longest_query_start = longest_query_start
                                    longest_query_end = ext_frag[1]
                                    longest_subject_start = longest_subject_start if longest_subject_start > \
    ...
    filter_TE_path = tmp_output_dir + '/longest_repeats.cons.filter_tandem.filter_fp.filter_derived.fa'
    with open(filter_TE_path, 'w') as f_save:
