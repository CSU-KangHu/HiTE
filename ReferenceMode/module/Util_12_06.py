...
def run_EAHelitron(flanking_len, temp_dir, cur_segments, EAHelitron, partition_index):
    candidate_Helitrons = {}
    all_copies_helitron_contigs = {}
    all_candidate_TIRs_path = temp_dir + '/' + str(partition_index) + '.fa'
    for item in cur_segments:
        query_name = item[0]
        cur_candidate_Helitrons = {}
        copies = item[1]
        for copy_index, copy in enumerate(copies):
            ...
            raw_end = copy[3] + flanking_len
            orig_seq = str(copy[4])
            # 如果有连续10个以上的N或者搜索的TSD有连续>=2个N，就过滤掉
            if orig_seq.__contains__('NNNNNNNNNN'):
                continue
            new_query_name = query_name + '-C_' + str(copy_index) + '-rawstart_' + str(raw_start) + '-rawend_' + str(raw_end)
            cur_candidate_Helitrons[new_query_name] = orig_seq
        all_copies_helitron_contigs.update(cur_candidate_Helitrons)

    store_fasta(all_copies_helitron_contigs, all_candidate_TIRs_path)
    EAHelitron_command = 'cd ' + temp_dir + ' && ' + 'perl ' + EAHelitron + '/EAHelitron -o ' + str(partition_index) + ' -u 20000 -T "ATC" -r 3 ' + all_candidate_TIRs_path
    ...
    for cur_name in all_copies_out_contigs.keys():
        ...
        raw_name = parts[0]
        query_name = raw_name.split('-C_')[0]
    ...
    for query_name in group_copies_contigs.keys():
        ...
        for name in cur_copies_out_contigs.keys():
            copy_index = int(name.split('-C_')[1].split('-')[0])
            ...
            seq_len = len(cur_seq)
            first_6bp = cur_seq[0: 6]
            if not copies_candidate.__contains__(copy_index):
                copies_candidate[copy_index] = (name, cur_distance, seq_len, first_6bp)
            else:
        ...
        # 遍历copies_candidate，统计出现次数最多的first 6bp in helitron
        for copy_index in copies_candidate.keys():
            item = copies_candidate[copy_index]
            cur_first_6bp = item[3]
            if not first_6bp_count.__contains__(cur_first_6bp):
                first_6bp_count[cur_first_6bp] = []
            query_names = first_6bp_count[cur_first_6bp]
            query_names.append(item[0])

        # 判断当前拷贝中哪一个first 6bp出现次数最多，那么该first 6bp对应的拷贝，最有可能是真实的Helitron
        max_count = 0
        max_count_first_6bp = ''
        for cur_first_6bp in first_6bp_count.keys():
            query_names = first_6bp_count[cur_first_6bp]
            if len(query_names) > max_count:
                max_count = len(query_names)
                max_count_first_6bp = cur_first_6bp
        # 取一条具有最多次数first 6bp的拷贝, 次数至少>=2
        if max_count_first_6bp != '' and max_count >= 2:
            query_names = first_6bp_count[max_count_first_6bp]
            first_query_name = query_names[0]
            seq = cur_copies_out_contigs[first_query_name]
            candidate_Helitrons[first_query_name] = seq
    return candidate_Helitrons
...
def multi_process_EAHelitron(all_copies, flanking_len, output, temp_dir, EAHelitron, threads):
...
def run_LTR_harvest(Genome_Tools_Home, reference, tmp_output_dir, ref_index, log):
    ...
    ltrharvest_command2 = Genome_Tools_Home + '/bin/gt ltrharvest -index ' + reference \
                          + ' -seed 20 -minlenltr 100 -maxlenltr 7000 -similar 85 -motif TGCA -mintsd 4 -maxtsd 6 ' \
                            '-vic 10 -seqids yes -longoutput > ' + tmp_output_dir + '/genome_'+str(ref_index)+'.fa.harvest.scn'
...
def determine_repeat_boundary_v1(repeats_path, longest_repeats_path, blast_program_dir,
                                 fixed_extend_base_threshold, max_single_repeat_len, reference, tmp_output_dir, threads):
            ...
            for longest_seq in seqs_tuples:
                ...
                if len(seq) >= 100 and not seq.__contains__('NNNNNNNNNN'):
                    f_save.write('>N_' + str(node_index) + '-len_' + str(len(seq)) +
                                 '-ref_' + chr_name + '_' + str(seq_ref_start) + '_' + str(seq_ref_end) + '\n' + seq + '\n')
                    node_index += 1
...
def multi_process_tsd(all_copies, tir_tsd_path, tir_tsd_dir, flanking_len, threads, TRsearch_dir, plant):
...
def flanking_copies(all_copies, query_path, reference, flanking_len, copy_num=10, query_coverage=0.99):
    ...
    for query_name in all_copies.keys():
        ...
        for i, copy in enumerate(copies):
            ...
            #需要判断拷贝的前5bp和后5bp是否与原始序列高度相似，只允许1 bp mismatch
...
def multiple_alignment_blast_and_get_copies(repeats_path, blast_program_dir, tools_dir, query_coverage):
    split_repeats_path = repeats_path[0]
    ref_db_path = repeats_path[1]
    blastn2Results_path = repeats_path[2]
    all_copies = None
    ...
    if len(repeat_contigs) > 0:
        ...
        all_copies = get_copies_v1(blastn2Results_path, split_repeats_path, ref_db_path, query_coverage=query_coverage)

    return all_copies

def run_blast_align(query_path, subject_path, output, blast_program_dir, flanking_len, flanking_region_distance):
    ...
    if file_exist(output):
        delete_name = judge_flank_align(flanking_region_distance, output, flanking_len)
    else:
        contignames, contigs = read_fasta(query_path)
        if len(contignames) > 0:
            query_parts = contignames[0].split('-c_')
            orig_query_name = query_parts[0]
            delete_name = orig_query_name
...
def search_confident_tir_batch(cur_segments, flanking_len, tir_tsd_dir, TRsearch_dir, partition_index, plant):
    ...
    for item in cur_segments:
        ...
        for copy_index, copy in enumerate(copies):
            ...
            tsd_search_distance = flanking_len
            cur_itr_contigs = search_confident_tir(orig_seq, tir_start, tir_end, tsd_search_distance, query_name, copy_index, plant)
    ...
    for query_name in group_copies_contigs.keys():
        ...
        is_fake_tsd = False
        # 1.过滤具有假阳性TSD的序列
        # 去掉那些在不同copy_index中多次（超过一半）出现的TSD，这些大概率不是真实的TSD，而是一个大重复序列包含了这个假TIR序列。
        ...
        # 3. 在所有具有TSD的拷贝中，真实TSD的长度应该多次出现在其他拷贝中。
        # 比如10条拷贝，8个具有TSD，真实的TSD长度为8，那么长度为8的TSD应该至少出现在一半（4条）以上有TSD的拷贝中
        # 如果真实TSD为TA、TTA或TAA、TTAA，那么其他具有TSD的拷贝应该绝大多数也是一样的TSD，否则为假阳性序列
        tsd_count = 0
        TA_tsd_count = 0
        min_TA_copy_index = 10000
        min_TA_query_name = ''

        TNN_tsd_count = 0
        min_TNN_copy_index = 10000
        min_TNN_query_name = ''

        TTAA_tsd_count = 0
        min_TTAA_copy_index = 10000
        min_TTAA_query_name = ''
        for copy_index in copies_candidate.keys():
            item = copies_candidate[copy_index]
            tsd_seq = item[2]
            ...
            if tsd_seq == 'TA':
                TA_tsd_count += 1
                if copy_index < min_TA_copy_index:
                    min_TA_copy_index = copy_index
                    min_TA_query_name = item[0]
            elif tsd_seq == 'TAA' or tsd_seq == 'TTA':
                TNN_tsd_count += 1
                if copy_index < min_TNN_copy_index:
                    min_TNN_copy_index = copy_index
                    min_TNN_query_name = item[0]
            elif tsd_seq == 'TTAA':
                TTAA_tsd_count += 1
                if copy_index < min_TTAA_copy_index:
                    min_TTAA_copy_index = copy_index
                    min_TTAA_query_name = item[0]
            else:
                cur_tsd_len = len(tsd_seq)
        ...
        # 真实TSD为TA、TTA或TAA、TTAA
        if float(TA_tsd_count)/tsd_count > 0.5:
            seq = cur_copies_out_contigs[min_TA_query_name]
            candidate_TIRs[min_TA_query_name] = seq
        elif float(TNN_tsd_count)/tsd_count > 0.5:
            seq = cur_copies_out_contigs[min_TNN_query_name]
            candidate_TIRs[min_TNN_query_name] = seq
        elif float(TTAA_tsd_count)/tsd_count > 0.5:
            seq = cur_copies_out_contigs[min_TTAA_query_name]
            candidate_TIRs[min_TTAA_query_name] = seq
        else:
            # 真实TSD为其他情况
            for cur_tsd_len in TSD_len_count.keys():
                query_names = TSD_len_count[cur_tsd_len]
                if float(len(query_names))/tsd_count > 0.5:
                    min_copy_index = 10000
                    min_query_name = ''
                    for name in query_names:
                        copy_index = int(name.split('-C_')[1].split('_')[0])
                        if copy_index < min_copy_index:
                            min_copy_index = copy_index
                            min_query_name = name
                    seq = cur_copies_out_contigs[min_query_name]
                    candidate_TIRs[min_query_name] = seq
        ...
        #         max_count = len(query_names)
        #         max_count_tsd_len = tsd_len
        # # 取具有最多次数TSD长度的拷贝, 次数至少>=2；同时因为我们的拷贝是按照与原序列长度最近的顺序排序的，因此我们选择copy_index最小的那条序列即可
...
def search_confident_tir(orig_seq, raw_tir_start, raw_tir_end, tsd_search_distance, query_name, copy_index, plant):
    itr_contigs = {}
    orig_seq_len = len(orig_seq)
    tir_starts = []
    tir_ends = []
...
def judge_flank_align(flanking_region_distance, output, flanking_len):
    cur_orig_query_name = ''
    ...
    with open(output, 'r') as f_r:
        for line in f_r:
            ...
            orig_query_name = query_parts[0]
            cur_orig_query_name = orig_query_name
...
def flank_region_align_v1(candidate_sequence_path, flanking_len, similar_ratio, reference, TE_type, tmp_output_dir, blast_program_dir, threads, log):
    log.logger.info('------generating candidate ' + TE_type + ' copies')
    starttime = time.time()
    tir_tsd_temp_dir = tmp_output_dir + '/' + TE_type + '_blast'
    all_copies = multi_process_align_and_get_copies(candidate_sequence_path, reference, blast_program_dir, tir_tsd_temp_dir, threads)
    ...
    with open(confident_copies_file, 'w') as f_save:
        f_save.write('# all copies have been flanked 50 bp at both ends\n')
    ...
    for index, orig_query_name in enumerate(new_all_copies.keys()):
        ...
        cur_query_copy_path = flank_align_dir + '/' + str(orig_query_name) + '_query.fa'
        store_fasta(cur_query_copies, cur_query_copy_path)
        cur_subject_copy_path = flank_align_dir + '/' + str(orig_query_name) + '_subject.fa'
        store_fasta(cur_subject_copies, cur_subject_copy_path)
        output_path = flank_align_dir + '/' + str(orig_query_name) + '.out'
        job = ex.submit(run_blast_align, cur_query_copy_path, cur_subject_copy_path, output_path, blast_program_dir, flanking_len, flanking_region_distance)
        jobs.append(job)
    ex.shutdown(wait=True)

    for job in as_completed(jobs):
        cur_query_name = job.result()
        if cur_query_name != '':
            deleted_names.append(cur_query_name)
            del new_all_copies[cur_query_name]
...
def multi_process_align_and_get_copies(query_path, subject_path, blast_program_dir, tmp_blast_dir, threads, is_removed_dir=True, query_coverage=0.99):
    tools_dir = ''
    if is_removed_dir:
        os.system('rm -rf ' + tmp_blast_dir)
    if not os.path.exists(tmp_blast_dir):
    ...
    jobs = []
    for file in longest_repeat_files:
        job = ex.submit(multiple_alignment_blast_and_get_copies, file, blast_program_dir, tools_dir, query_coverage)
        jobs.append(job)
    ex.shutdown(wait=True)
