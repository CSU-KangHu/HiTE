...
def run_HelitronScanner(sh_dir, temp_dir, cur_segments, HSDIR, HSJAR):
    candidate_Helitrons = {}
    for item in cur_segments:
        query_name = item[0]
        cur_candidate_Helitrons = {}
        cur_candidate_Helitrons_path = temp_dir + '/' + str(query_name) + '.fa'
        copies = item[1]
        for copy_index, copy in enumerate(copies):
            ...
            if orig_seq.__contains__('NNNNNNNNNN'):
                continue
            new_query_name = query_name + '-C_' + str(copy_index)
        ...
        HelitronScanner_command = 'cd ' + temp_dir + ' && ' + 'sh ' + sh_dir + '/run_helitron_scanner.sh ' \
                                  + str(query_name) + ' ' + cur_candidate_Helitrons_path + ' ' + HSDIR + ' ' + HSJAR
        os.system(HelitronScanner_command + '> /dev/null 2>&1')

        cur_helitron_out = temp_dir + '/' + str(query_name) + '.HelitronScanner.draw.hel.fa'
        cur_rc_helitron_out = temp_dir + '/' + str(query_name) + '.HelitronScanner.draw.rc.hel.fa'
...
def multi_process_helitronscanner(all_copies, output, sh_dir, temp_dir, HSDIR, HSJAR, threads):
    ...
    objs = []
    for partition_index, cur_segments in enumerate(segments_cluster):
        obj = ex.submit(run_HelitronScanner, sh_dir, temp_dir, cur_segments, HSDIR, HSJAR)
        objs.append(obj)
        job_id += 1
    ex.shutdown(wait=True)
    candidate_Helitrons = {}
...
def flanking_copies(all_copies, reference, flanking_len, copy_num=10):
    new_all_copies = {}
    ref_names, ref_contigs = read_fasta(reference)
    for query_name in all_copies.keys():
        copies = all_copies[query_name]
...
def run_blast_align(query_path, subject_path, output, blast_program_dir):
...
def flank_region_align_v1(candidate_sequence_path, flanking_len, similar_ratio, reference, TE_type, tmp_output_dir, blast_program_dir, threads, log):
    ...
    for index, orig_query_name in enumerate(new_all_copies.keys()):
        ...
        if len(cur_query_copies) > 0 and len(cur_subject_copies) > 0:
            pool.apply_async(run_blast_align, (cur_query_copy_path, cur_subject_copy_path, output_path, blast_program_dir,))
    pool.close()
    pool.join()
    ...
    for index, orig_query_name in enumerate(new_all_copies.keys()):
        ...
        if not os.path.exists(output_path):
            continue
        job = ex.submit(judge_flank_align, flanking_region_distance, output_path, flanking_len)
        jobs.append(job)
    ex.shutdown(wait=True)
...
def remove_ltr_from_tir(confident_tir_path, confident_ltr_cut_path, all_copies):
    # copy -> (subject_name, subject_start, subject_end, query[2], direct)
    query_names, query_contigs = read_fasta(confident_tir_path)
    subject_names, subject_contigs = read_fasta(confident_ltr_cut_path)
    for query_name in all_copies.keys():
        copies = all_copies[query_name]
        for copy in copies:
            subject_name = copy[0]
            ...
            if float(alignment_len) / subject_len >= 0.95:
                ...
                break
    store_fasta(query_contigs, confident_tir_path)
