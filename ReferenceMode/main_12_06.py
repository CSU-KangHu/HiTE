...
if __name__ == '__main__':
    ...
    for ref_index, cut_reference in enumerate(cut_references):
        ...
        longest_repeats_flanked_copies_file = tmp_output_dir + '/longest_repeats_' + str(ref_index) + '.flanked.copies'
        ...
        if not is_recover or not file_exist(resut_file) or not file_exist(longest_repeats_flanked_copies_file):
            starttime = time.time()
            ...
            determine_repeat_boundary_v1(repeats_path, longest_repeats_path, blast_program_dir,
                                         fixed_extend_base_threshold, max_repeat_len, reference, tmp_output_dir, threads)
        ...
        if not is_recover or not file_exist(resut_file):
            ...
            tir_identification_command = 'cd ' + test_home + ' && python ' + test_home + '/judge_TIR_transposons.py -g ' \
                                         + cut_reference + ' --copies ' + longest_repeats_flanked_copies_file\
                                         + ' -t ' + str(threads)+' --TRsearch_dir ' + TRsearch_dir \
                                         + ' --tmp_output_dir ' + tmp_output_dir + ' --blast_program_dir ' \
        ...
        if not is_recover or not file_exist(resut_file):
            ...
            helitron_identification_command = 'cd ' + test_home + ' && python ' + test_home + '/judge_Helitron_transposons.py --copies ' \
                                              + longest_repeats_flanked_copies_file + ' -g ' + cut_reference + ' -t ' + str(threads) \
        ...
        resut_file = tmp_output_dir + '/confident_other'+str(ref_index)+'.fa'
        if not is_recover or not file_exist(resut_file):
            starttime = time.time()
        ...
        if not is_recover or not file_exist(resut_file):
            ...
            backjob = multiprocessing.Process(target=run_LTR_harvest,
                                              args=(Genome_Tools_Home, cut_reference, tmp_output_dir, ref_index, log,))
            backjob.start()
        else:
            log.logger.info(resut_file + ' exists, skip...')
        ...
        if not is_recover or not file_exist(candidate_ltr_path) or not file_exist(candidate_ltr_cut_path):
            starttime = time.time()
            ...
            ltr_finder_output = ref_rename_path + '.finder.combine.scn'
        ...
        if backjob is not None:
            ...
            candidate_ltr_path = tmp_output_dir + '/candidate_ltr_harvest_' + str(ref_index) + '.fa'
            candidate_ltr_cut_path = tmp_output_dir + '/candidate_ltr_harvest_cut_' + str(ref_index) + '.fa'
            store_LTR_seq(ltrharvest_output, cut_reference, candidate_ltr_path, candidate_ltr_cut_path)

        resut_file = tmp_output_dir + '/confident_ltr_' + str(ref_index) + '.fa'
        confident_ltr_cut_path = tmp_output_dir + '/confident_ltr_cut_' + str(ref_index) + '.fa'
        if not is_recover or not file_exist(resut_file) or not file_exist(confident_ltr_cut_path):
            starttime = time.time()
            log.logger.info('Start step2.3: determine fine-grained LTR')
            ltr_identification_command = 'cd ' + test_home + ' && python ' + test_home + '/judge_LTR_transposons.py -g ' \
                                         + ref_rename_path + ' -t ' + str(threads) + ' --tmp_output_dir ' + tmp_output_dir \
                                         + ' --blast_program_dir ' + blast_program_dir \
                                         + ' --TRF_Path ' + TRF_Path + ' --tandem_region_cutoff ' \
                                         + str(tandem_region_cutoff) + ' --flanking_len ' + str(flanking_len) \
                                         + ' --ref_index ' + str(ref_index)
            os.system(ltr_identification_command)
            endtime = time.time()
            dtime = endtime - starttime
        ...
        cur_confident_other_path = tmp_output_dir + '/confident_other'+str(ref_index)+'.fa'
    ...
    for ref_index, cut_reference in enumerate(cut_references):
        ...
        os.system('cat ' + cur_confident_ltr_path + ' >> ' + confident_ltr_path)
    ...
    all_copies = multi_process_align_and_get_copies(confident_ltr_cut_path, confident_tir_path, blast_program_dir, temp_dir, threads, query_coverage=0.8)
    ...
    all_copies = multi_process_align_and_get_copies(confident_ltr_path, confident_tir_path, blast_program_dir,
                                                    temp_dir, threads, query_coverage=0.8)
