...
# def getQueryRecords(repeats_path, reference, threads, tools_dir, use_align_tools, tmp_output_dir, RepeatMasker_Home):
...
#             records.append((reference_name, alignment_len, t_start, t_end))
#             query_records[query_name] = records
#     elif use_align_tools == 'RepeatMasker':
#         rm_command = RepeatMasker_Home + '/RepeatMasker -pa ' + str(threads) + ' -q -no_is -norna -nolow -lib ' + repeats_path + ' ' + reference
#         os.system(rm_command)
#         rm_out = tmp_output_dir + '/repeats.filter1.out'
#         os.system('cp ' + reference + '.out ' + rm_out)
#         rm_out_tab = tmp_output_dir + '/repeats.filter1.out.tab'
...
#                     repeat_end = int(parts[12])
#                 alignment_len = repeat_end-repeat_start
#                 if not query_records.__contains__(repeat_name):
#                     query_records[repeat_name] = []
#                 ref_pos = query_records[repeat_name]
#                 ref_pos.append((chr_name, alignment_len, chr_start, chr_end))
#
#     return query_records


# def connect_frags(connected_regions, repeats_path, reference, threads, tools_dir, tmp_output_dir, skip_threshold, RepeatMasker_Home):
#     repeat_contignames, repeat_contigs = read_fasta(repeats_path)
#     #use_align_tools = 'bwa'
#     use_align_tools = 'RepeatMasker'
#     old_query_records = getQueryRecords(repeats_path, reference, threads, tools_dir, use_align_tools, tmp_output_dir, RepeatMasker_Home)
#
#     # filter repeat with fewer copies, it maybe segmental duplication
#     query_records = {}
#     filter_segmental_copies = 0
#     for repeat_name in old_query_records.keys():
#         ref_pos = old_query_records[repeat_name]
#         if len(ref_pos) >= filter_segmental_copies:
#             query_records[repeat_name] = ref_pos
#
...
#                 continue
#             # alignment length > 50% of the repeat length, is a valid element
#             if float(alignment_len)/query_len >= 0.5:
