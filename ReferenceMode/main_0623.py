...
# backjob.start()
    ...
    # filter false positive, strategy on 2022/06/21
    tmp_blast_dir = tmp_output_dir + '/tmp_blast'
    #os.system('rm -rf ' + tmp_blast_dir)
    if not os.path.exists(tmp_blast_dir):
        os.makedirs(tmp_blast_dir)
    ...
    for file in longest_repeats_files:
        job = ex.submit(get_flanking_instances, file, blast_program_dir, tools_dir, extend_base_threshold, flanking_len)
        jobs.append(job)
    ex.shutdown(wait=True)

    for job in as_completed(jobs):
        cur_instances = job.result()
        for query_name in cur_instances.keys():
            instances[query_name] = cur_instances[query_name]

    instance_path = tmp_output_dir + '/instance.fa'
    node_index = 0
    with open(instance_path, 'w') as f_save:
        for query_name in instances.keys():
            for repeat in instances[query_name]:
                instance_name = 'I_' + str(node_index) + '-len_' + str(len(repeat))
                f_save.write('>' + instance_name + '\n' + repeat + '\n')
                node_index += 1

    instance_consensus = tmp_output_dir + '/instance.cons.fa'
    cd_hit_command = tools_dir + '/cd-hit-est -aS ' + str(0.95) + ' -aL ' + str(0.95) + ' -c ' + str(0.95) + ' -G 0 -g 1 -A 80 -i ' + instance_path + ' -o ' + instance_consensus + ' -T 0 -M 0'
    log.logger.debug(cd_hit_command)
    os.system(cd_hit_command)

    instanceNames, instanceContigs = read_fasta(instance_consensus)
    instance_files = []
    segments_cluster = divided_array(list(instanceContigs.items()), threads)
    for partition_index, cur_segments in enumerate(segments_cluster):
        single_tmp_dir = tmp_blast_dir + '/' + str(partition_index)
        if not os.path.exists(single_tmp_dir):
            os.makedirs(single_tmp_dir)
        split_instance_file = single_tmp_dir + '/instance_split.fa'
        cur_contigs = {}
        for item in cur_segments:
            cur_contigs[item[0]] = item[1]
        store_fasta(cur_contigs, split_instance_file)
        os.system('cp ' + reference + ' ' + single_tmp_dir)
        instance_files.append((split_instance_file, single_tmp_dir + '/' + ref_filename,
                                      single_tmp_dir + '/instance.ref.out'))

    ex = ProcessPoolExecutor(threads)
    instances = {}
    jobs = []
    for file in instance_files:
        job = ex.submit(filter_false_positive, file, blast_program_dir, tools_dir, extend_base_threshold, flanking_len)
        jobs.append(job)
    ex.shutdown(wait=True)
