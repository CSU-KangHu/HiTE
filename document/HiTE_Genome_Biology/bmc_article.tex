...
Long-read sequencing technology, able to cross the repetitive regions in the genome, is greatly improving the quality of genome assembly. The high-quality assemblies give a bright future to comprehensive annotation of TEs. Although numerous methods exist for the annotation of comprehensive TEs or only for specific classes of TEs,  the highly variable TEs make automated TE discovery and annotation challenging and time-consuming. In addition, all automatically generated TE libraries require extensive manual editing due to the inability to determine the true ends of TEs. Therefore, an automated and high-precision TE annotation tool that can produce structurally intact and non-redundant TE libraries is urgently needed.

\parttitle{Results} %if any
We have developed an ensemble method for high-precision transposable element annotation, known as HiTE, which can be used to annotate almost all types of TE. Using the complementary benchmarking methods released in two recent studies, HiTE  achieved the highest precision in TE annotation and restored the most number of perfect gold standard sequences on four different model species. Moreover, HiTE can discover novel TEs with low copy numbers that are not included in known libraries. 
...
\begin{table}[h!]
	...
	\begin{tablenotes}
		...
		\item[3] *Danio rerio ``assembly GRCz11"
		\item[4] *Drosophila melanogaster ``assembly Release 6 plus ISO1 MT"
	\end{tablenotes}
	}
\end{table}
