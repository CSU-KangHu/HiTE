...
from Util import read_fasta, store_fasta, Logger, multi_process_tsd, rename_fasta, file_exist, \
    run_itrsearch, get_short_tir_contigs, flank_region_align_v5


def is_transposons(filter_dup_path, reference, threads, tmp_output_dir, ref_index, log, subset_script_path, plant, debug, TRsearch_dir, split_ref_dir, is_recover):
    ...
    for i in range(iter_num):
        ...
        if not is_recover or not file_exist(resut_file):
            flank_region_align_v5(input_file, output_file, flanking_len, reference, split_ref_dir, TE_type, tmp_output_dir, threads,
                                  ref_index, log, subset_script_path, plant, debug, i, result_type)
    ...
    rename_fasta(confident_tir_cons, confident_tir_path, 'TIR_' + str(ref_index))
